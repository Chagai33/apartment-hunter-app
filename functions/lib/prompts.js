...
<rules>
  ...
  <rule_2_floor_mapping>
    ...
    CRITICAL: If the text states "קומת קרקע" (Karka) or "פרטר" (Parter) without numeric modifiers, YOU MUST output the integer 0 for the floor. 
  ...
  <rule_4_broker_fees>
    ...
    CRITICAL: If the text says "ללא תיווך" (no broker), "פרטי" (private), or "מפרטי" (from a private owner), YOU MUST explicitly set the 'brokerFee' boolean to false in your JSON output.
  ...
  <rule_6_boolean_omissions>
    CRITICAL: For all optional boolean capabilities (e.g., parking, elevator, balcony, renovated), DO NOT GUESS. 
    If a feature is not explicitly mentioned in the text, you MUST omit it (return null). 
    Only return 'false' if the text EXPLICITLY denies the feature (e.g., "אין חניה", "ללא מעלית").
  </rule_6_boolean_omissions>
</rules>
...
<dictionary>
  ...
  <term concept="Furnished">מרוהט, מרוהטת, מרוהטת קומפלט, ריהוט מלא, ריהוט חלקי, פול מרוהט</term>
  <term concept="Balcony">מרפסת, מרפסת שמש, מרפסת סוכה</term>
  <term concept="Yard">חצר, עורפית, גינה</term>
  <term concept="Bills Included">כולל הכל, חשמל מים וארנונה, כולל חשבונות</term>
  <term concept="Shelter">מקלט, מקלט בבניין, ממ"ק</term>
...
<catch_all_instructions>
  Scan the text for unique localized architectural features and amenities (e.g., חצר, גינה, דוד שמש, סורגים, בוידם, יחידת הורים, רשתות, מקלט, מקלט בבניין, כולל הכל).
  Add these EXACTLY AS THEY APPEAR in Hebrew as string items in the 'inferredCustomFeatures' array.
