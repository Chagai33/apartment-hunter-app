...
export const apartmentSchema = z.object({
    ...
    neighborhood: z.string().nullable().optional().describe("The neighborhood the apartment is in. E.g. if the text mentions 'שכונת תל חיים', output 'תל חיים'."),
    ...
    elevator: z.boolean().nullable().optional().describe("Set to true if 'מעלית' is mentioned. Set to false ONLY if explicitly denied ('ללא מעלית', 'אין מעלית'). Otherwise, return null."),
    parking: z.boolean().nullable().optional().describe("Set to true if 'חניה' or 'חניון' is mentioned. Set to false ONLY if explicitly denied ('ללא חניה', 'אין חניה'). Otherwise, return null."),
    balcony: z.boolean().nullable().optional().describe("Set to true if 'מרפסת' is mentioned. Set to false ONLY if explicitly denied ('ללא מרפסת', 'אין מרפסת'). Otherwise, return null."),
    ac: z.boolean().nullable().optional().describe("Set to true if 'מזגן' or 'מיזוג' is mentioned. Set to false ONLY if explicitly denied ('ללא מזגן', 'אין מזגן'). Otherwise, return null."),
    tama38: z.boolean().nullable().optional().describe("True if 'ממ\"ד' (Mamad) is explicitly mentioned. False ONLY if explicitly stated there is no Mamad. Otherwise, return null. (Note: Miklat/Mamak are NOT Mamad)."),
    pets: z.boolean().nullable().optional().describe("True if pets are allowed ('חיות מחמד', 'בעלי חיים'). False ONLY if explicitly denied ('ללא בעלי חיים', 'אסור חיות'). Otherwise, return null."),
    renovated: z.boolean().nullable().optional().describe("True if 'משופץ' is mentioned. False ONLY if explicitly stated it needs renovation. Otherwise, return null."),
    furnished: z.boolean().nullable().optional().describe("True if furnished ('מרוהט'). False ONLY if explicitly stated empty/unfurnished ('ללא ריהוט', 'ריקה'). Otherwise, return null."),
    ...
    brokerFee: z.boolean().nullable().optional().describe("CRITICAL: If the text indicates NO broker fee ('ללא תיווך', 'פרטי', 'מפרטי'), you MUST output explicitly false. Set to true if a professional broker is involved ('מתווך', 'עמלת תיווך')."),
    ...
    inferredCustomFeatures: z.array(z.string()).nullable().optional().describe("An array of other specifically inferred features. Items MUST be the EXACT Hebrew phrases found in text, e.g. ['דוד שמש', 'סורגים', 'חצר', 'מקלט בבניין', 'כולל הכל']. Map uniquely localized architectural features here. Only add items if explicitly mentioned.")
});
...
export const geminiResponseSchema: Schema = {
    ...
    properties: {
        ...
        neighborhood: { type: "STRING" as any, description: "The neighborhood the apartment is in. E.g. if the text mentions 'שכונת תל חיים', output 'תל חיים'.", nullable: true },
        ...
        floor: { type: "NUMBER" as any, description: "Extract the floor level as an integer. CRITICAL: 'קומת קרקע' (Ground) or 'פרטר' (Parter) MUST strictly evaluate to the number 0. 'מרתף' (Basement) MUST evaluate to -1. If a range is given, extract only the specific floor.", nullable: true },
        ...
        elevator: { type: "BOOLEAN" as any, description: "Set to true if 'מעלית' is mentioned. Set to false ONLY if explicitly denied ('ללא מעלית', 'אין מעלית'). Otherwise, return null.", nullable: true },
        parking: { type: "BOOLEAN" as any, description: "Set to true if 'חניה' or 'חניון' is mentioned. Set to false ONLY if explicitly denied ('ללא חניה', 'אין חניה'). Otherwise, return null.", nullable: true },
        balcony: { type: "BOOLEAN" as any, description: "Set to true if 'מרפסת' is mentioned. Set to false ONLY if explicitly denied ('ללא מרפסת', 'אין מרפסת'). Otherwise, return null.", nullable: true },
        ac: { type: "BOOLEAN" as any, description: "Set to true if 'מזגן' or 'מיזוג' is mentioned. Set to false ONLY if explicitly denied ('ללא מזגן', 'אין מזגן'). Otherwise, return null.", nullable: true },
        tama38: { type: "BOOLEAN" as any, description: "True if 'ממ\"ד' (Mamad) is explicitly mentioned. False ONLY if explicitly stated there is no Mamad. Otherwise, return null. (Note: Miklat/Mamak are NOT Mamad).", nullable: true },
        pets: { type: "BOOLEAN" as any, description: "True if pets are allowed ('חיות מחמד', 'בעלי חיים'). False ONLY if explicitly denied ('ללא בעלי חיים', 'אסור חיות'). Otherwise, return null.", nullable: true },
        renovated: { type: "BOOLEAN" as any, description: "True if 'משופץ' is mentioned. False ONLY if explicitly stated it needs renovation. Otherwise, return null.", nullable: true },
        furnished: { type: "BOOLEAN" as any, description: "True if furnished ('מרוהט'). False ONLY if explicitly stated empty/unfurnished ('ללא ריהוט', 'ריקה'). Otherwise, return null.", nullable: true },
        ...
        brokerFee: { type: "BOOLEAN" as any, description: "CRITICAL: If the text indicates NO broker fee ('ללא תיווך', 'פרטי', 'מפרטי'), you MUST output explicitly false. Set to true if a professional broker is involved ('מתווך', 'עמלת תיווך').", nullable: true },
